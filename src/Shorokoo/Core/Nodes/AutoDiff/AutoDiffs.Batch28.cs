...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    ...
    internal static partial class AutoDiffs
    {
        ...
        // non-differentiable — and the two float feeds additionally take optional float32
        // distribution inputs (low/high for the uniform, mean/scale for the normal), which are
        // ordinary graph values and CAN therefore sit on a reachable autograd path (a model
        // computing its distribution from a trainable parameter and training through the draw).
        // None of these ops has an [AutoDiff] entry, so such a model fails loud —
        // AutoDiffNotSupportedException [AD003] — rather than silently getting a wrong gradient.
        // Reparameterizing a draw w.r.t. its distribution is not implemented; add an entry here
        // if it ever is.
