...
| `.skpt` / `Persistence` | Shorokoo's single-file checkpoint container (a STORED zip with a `config.json` manifest); `Persistence.From(...).Save(...)` / `Persistence.Load` save and reload a concrete model with its weights. | `skpt-checkpoints.md` |
