...
namespace Shorokoo.Core.Interpreter.Helpers;
...
internal static class TensorDataConverter
{
        ...
        Variable? reference)
    {
        ...
        // DType.Utf8 is variable-length UTF-8; the underlying ORT tensor has no flat
        ...
        if (shape.Count <= maxElements && dtype != DType.Utf8 && rawBytes(source) is { } bytes)
        {
