...
| `Shorokoo.Core.Inference.Abstractions` | `InferenceBackend`, `ComputeDevice`, `DeviceMemorySettings` and `RunSettings` (what a `ComputeContext` carries for the sessions it compiles and for each run they make), `DeviceMemory` (the readings), and the `Float16`/`BFloat16` element types | [inference.md](inference.md), [core-types.md](core-types.md) |
