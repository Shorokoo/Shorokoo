...
- `stage` records the graph's `GraphKind` (see
  ...
  foreign data), so loaders can refuse a mismatched file at load time, naming the
  file, rather than leaving it to the execution gate to refuse the graph later.
  Pass the optional `requiredStage` argument to enforce it:
