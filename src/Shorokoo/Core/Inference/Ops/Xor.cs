...
internal sealed class XorOp : QuickOp
{
    public override string OpCode => OpCodes.XOR;
    ...
    protected override RuntimeTensor[] Compute(RuntimeTensor?[] inputs, OnnxCSharpAttributes attrs, int maxDataElements)
    {
        ...
        if (shape is not null && RuntimeTensorFactory.ShouldStoreData(shape, maxDataElements)
            && a?.BoolData is { } ad && b?.BoolData is { } bd
            && a.Shape is not null && b.Shape is not null)
        {
            var data = ElementwiseBroadcast.Bool(ad, a.Shape, bd, b.Shape, shape, (x, y) => x ^ y);
            return [rt with { BoolData = System.Collections.Immutable.ImmutableArray.Create(data) }];
        }
        return [rt];
    }
}
