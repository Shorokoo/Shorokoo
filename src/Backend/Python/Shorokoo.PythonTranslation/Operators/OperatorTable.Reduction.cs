...
// Reductions; the semantics are in each support package's ops_reduction.py.
internal static partial class OperatorTable
{
    static partial void RegisterReduction(Registry table)
    {
        ...
        table.Map("ArgMax", M + "arg_max", argAttributes, gradient: GradientRule.NotDifferentiable);
        table.Map("ArgMin", M + "arg_min", argAttributes, gradient: GradientRule.NotDifferentiable);
    }
}
