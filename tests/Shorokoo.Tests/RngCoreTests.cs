...
public class RngCoreTests
{
    ...
    [Fact]
    public void TestKeyDerivationIsNameDerivedAndStable()
    {
        var cfg = new RngConfig { MasterSeed = 20260702 };
        var k1 = cfg.ResolveKey64(RngCollection.Params, "backbone.block3.conv1.weight");
        var k2 = cfg.ResolveKey64(RngCollection.Params, "backbone.block3.conv1.weight");
        var k3 = cfg.ResolveKey64(RngCollection.Params, "backbone.block3.conv2.weight");
        Assert.Equal(k1, k2);                                 // stable for a name
        Assert.NotEqual(k1, k3);                              // different names differ
        // Same name in a different collection is a different stream.
        Assert.NotEqual(k1, cfg.ResolveKey64(RngCollection.Runtime, "backbone.block3.conv1.weight"));
    }
    ...
    public void TestMasterSeedChangeRerandomizesEveryStream()
    {
        ...
        Assert.NotEqual(a.ResolveKey64(RngCollection.Params, "w"),
                        b.ResolveKey64(RngCollection.Params, "w"));
    }
    ...
    public void TestOverrideIsolatesASingleStream()
    {
        ...
        var cfg = new RngConfig { MasterSeed = 7 }
            .Override(RngConfig.StreamName(RngCollection.Params, "w1"), seed: 1234);

        // The overridden stream changes; an untouched stream keeps its derived key.
        Assert.NotEqual(baseCfg.ResolveKey64(RngCollection.Params, "w1"),
                        cfg.ResolveKey64(RngCollection.Params, "w1"));
        Assert.Equal(baseCfg.ResolveKey64(RngCollection.Params, "w2"),
                     cfg.ResolveKey64(RngCollection.Params, "w2"));
        Assert.True(cfg.HasOverride(RngCollection.Params, "w1"));
        Assert.False(cfg.HasOverride(RngCollection.Params, "w2"));
    }
}
