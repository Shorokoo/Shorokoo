...
internal sealed class NodeContext
{
    ...
    public string Constant(TorchConstant constant) => _translator.AddConstant(constant);
    ...
    public TorchUnsupportedModelException Unsupported(string what)
        => new(TorchUnsupportedReason.UnsupportedUsage, Node.Domain, Node.OpType,
            $"The PyTorch backend cannot run the {Node.OpType} node '{Node.Name}': {what}.");
    ...
    /// </summary>
    /// <exception cref="TorchUnsupportedModelException">The node carries an attribute not in
