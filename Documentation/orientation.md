...
| `Shorokoo.Core.Inference.Abstractions` | `InferenceBackend`, `ComputeDevice`, `DeviceMemorySettings` and `RunSettings` (the per-session and per-run device-memory settings a `ComputeContext` carries), `DeviceMemory` (the readings), and the `Float16`/`BFloat16` element types | [inference.md](inference.md), [core-types.md](core-types.md) |
