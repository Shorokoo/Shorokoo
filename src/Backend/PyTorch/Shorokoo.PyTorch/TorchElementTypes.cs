...
/// <summary>
/// The element types a PyTorch tensor can hold, and how many bytes each lays down.
///
/// <para>PyTorch has every fixed-stride ONNX type, including the four 8-bit float kinds and both
/// complex ones, which ONNX Runtime does not build from bytes. So the byte table is
...
/// in its words.</para>
/// </summary>
internal static class TorchElementTypes
{
    /// <summary>The bytes one element occupies.</summary>
    /// <exception cref="NotSupportedException">The type has no fixed stride, or no torch dtype.</exception>
