...
[Init draws inside loops](#init-draws-inside-loops).
...
re-seeds that iteration only; sibling iterations keep their derived keys. The per-trip streams of
an **init draw inside a loop** are the exception: they hang off the parameter's key rather than
off ModelId slots of their own, so they have no address of their own. A `Params` override on that
parameter re-seeds all of its trips together, coherently. An override that
...
<a id="init-draws-inside-loops"></a>
...
are independent samples — the same rule a runtime feed in a loop follows, and the same one
`Documentation/nn-library.md` states for initializer authors. Every enclosing loop contributes an
index, so a draw nested two deep varies with both.

The trips share the parameter's stream and are reproducible for a config like everything else; what
they do not have is individual override addresses, as noted under
[Per-stream overrides](#per-stream-overrides).
...
To give two parameters identical initial values — matching a hand-built reference layer —
override each stream to the same seed: `Override(RngCollection.Params, path, s)` on both.
