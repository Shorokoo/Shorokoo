...
/// (missing shard, missing tensor, duplicate name, LFS pointer).
/// </summary>
[Trait("Domain", "Core")]
...
public class SafeTensorShardingCoverageTests
{
    ...
    public void TestShardedValidationFailures()
    {
        ...
        try
        {
            ...
            // A shard that is a git-LFS pointer instead of real content.
            File.WriteAllText(Path.Combine(dir, "lfs.safetensors"),
                "version https://git-lfs.github.com/spec/v1\noid sha256:abc\nsize 123\n");
            WriteIndex("{\"l\":\"lfs.safetensors\"}");
            Assert.Throws<InvalidDataException>(() => SafeTensorLoader.LoadSafeTensors(indexPath));
