...
internal sealed class TransposeOp : QuickOp
{
    public override string OpCode => OpCodes.TRANSPOSE;
    ...
    protected override RuntimeTensor[] Compute(RuntimeTensor?[] inputs, OnnxCSharpAttributes attrs, int maxDataElements)
    {
        ...
        // Default perm reverses the dimensions per spec.
        var permAttr = attrs.GetIntsVal(OnnxOpAttributeNames.AttrPerm);
        int[] perm;
        if (permAttr is null)
        {
            perm = new int[rank];
        ...
        {
            if (permAttr.Length != rank || permAttr.Any(p => p < 0 || p >= rank))
                return [RuntimeTensorFactory.Create(dtype, null)]; // invalid perm — don't guess
            perm = permAttr;
        }

        var newDims = perm.Select(p => inDims[p]).ToArray();
        ...
        {
            long src = 0;
            for (int d = 0; d < rank; d++) src += idx[d] * inStrides[perm[d]];
