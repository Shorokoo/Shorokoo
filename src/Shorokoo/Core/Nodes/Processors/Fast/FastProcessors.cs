...
namespace Shorokoo.Core.Nodes.Processors.Fast
{
    ...
    internal static class FastInlineModulesAndFunctions
    {
        public static void Process(InternalComputationGraph graph)
        {
            ...
            // The model and the call site each spliced feed came from. Collected across every pass
            // because a model's call sites need not share one: a call reached through a
            // [Hyper] Model<> only enters the graph when the enclosing body is spliced, a pass
            // after a direct call was (Shorokoo/Shorokoo#311). Splitting the streams therefore
            // waits until inlining is done and every call site is in hand.
            var feedSites = new Dictionary<FastNodeKey, (FastNodeKey creation, FastNodeKey invoke)>();
            while (ProcessOnePass(graph, feedSites)) { }
            SplitFeedStreamsPerCallSite(graph, feedSites);
        }

        private static bool ProcessOnePass(
            InternalComputationGraph graph,
            Dictionary<FastNodeKey, (FastNodeKey creation, FastNodeKey invoke)> feedSites)
        {
            var nodeByKey = FastProcessorHelper.BuildNodeByKey(graph);
            ...
            foreach (var fastNode in graph.Nodes)
            {
                ...
                else
                {
                    ...
                    if (directModelCreation is not null)
                    {
                        ...
                        // Only a call site whose model the walk named can collide with another:
                        // the model-variable route below keys off the variable, whose path carries
                        // a runtime component per id element and so already separates the models a
                        // position picks between.
                        foreach (var subNode in subFastGraph.Nodes)
                            if (IsRngFeed(subNode))
                                feedSites[subNode.Key] = (directModelCreation.Key, fastNode.Key);
                    }
                    else
                    {
        ...
        /// <summary>True for the three keyed draw sites, whose ModelId names an RNG stream.</summary>
        private static bool IsRngFeed(FastNode node)
            => node.OpCode == InternalOpCodes.SHRK_RANDOM_UNIFORM
            || node.OpCode == InternalOpCodes.SHRK_RANDOM_NORMAL
            || node.OpCode == InternalOpCodes.SHRK_RANDOM_BITS;

        /// <summary>
        /// Gives each call site of a model called more than once its own RNG streams.
        ///
        /// <para>Calling one model object twice shares its id, and so its parameters — that is what
        /// calling it twice is for. A draw is not a value it holds: each execution of a draw is a
        /// fresh sample, so two call sites must not share a stream (Shorokoo/Shorokoo#298). Feeds
        /// that come out of inlining with the same ModelId are exactly the ones that would, so each
        /// call site's copy takes a <see cref="ModelId.CallSiteIndex"/> component naming its
        /// ordinal.</para>
        ///
        /// <para>Only the feeds take it: the model's parameter ids stay collapsed, since sharing
        /// the weights across the calls is the point. And only a model actually called more than
        /// once takes it, so a single-call model keeps the path it already had and any
        /// <c>Rng.Pin</c> against it stays valid — adding a second call is what splits the stream,
        /// and it names both halves, because one stream cannot become two while either keeps the
        /// old name.</para>
        /// </summary>
        private static void SplitFeedStreamsPerCallSite(
            InternalComputationGraph graph,
            Dictionary<FastNodeKey, (FastNodeKey creation, FastNodeKey invoke)> feedSites)
        {
            // Feeds by the model they were spliced from, in node order.
            var byCreation = new Dictionary<FastNodeKey, List<FastNode>>();
            foreach (var node in graph.Nodes)
            {
                if (!IsRngFeed(node)) continue;
                if (!feedSites.TryGetValue(node.Key, out var site)) continue;
                if (!byCreation.TryGetValue(site.creation, out var list))
                    byCreation[site.creation] = list = [];
                list.Add(node);
            }

            foreach (var (_, feeds) in byCreation)
            {
                var ordinalOfSite = new Dictionary<FastNodeKey, int>();
                foreach (var feed in feeds)
                {
                    var invoke = feedSites[feed.Key].invoke;
                    if (!ordinalOfSite.ContainsKey(invoke)) ordinalOfSite[invoke] = ordinalOfSite.Count;
                }
                if (ordinalOfSite.Count < 2) continue;   // called once; its path does not move

                foreach (var feed in feeds)
                {
                    var attrs = feed.Attributes.GetAttributeVals().ToDictionary();
                    var idVals = (long[])attrs[OnnxOpAttributeNames.ShrkAttrLocalModelId]!;
                    attrs[OnnxOpAttributeNames.ShrkAttrLocalModelId] =
                        (long[])[.. idVals, ModelId.CallSiteIndex, ordinalOfSite[feedSites[feed.Key].invoke]];
    ...
    {
        /// <summary>
        /// Returns the <c>MODEL_PARAM_ID_REF</c> sites whose initializer values QEE could not
        /// resolve. Such a site produces no candidate model id, so the rewrite below fills it with
        /// an empty CONSTANT and the parameter silently vanishes from the architecture — see
        /// <see cref="ThrowIfAParamShapeNeedsAnUnhintedInput"/>, which the caller runs on these.
        /// </summary>
        public static IReadOnlyList<FastNode> Process(
            InternalComputationGraph graph,
            ...
            ComputeContext? computeContext = null)
        {
            ...
            if (candidateModelIdInfos.IsEmpty) return unresolvedSites;
        ...
        /// resolve. When the reason is user error — no hint for an input whose <em>value</em> the
        /// definition needs, e.g. <c>InitSimple.Init(input.ShapeTensor())</c>, leaving QEE nothing
        /// to evaluate it from — say so, naming that input, rather than dropping the parameter
        /// silently or reporting the pipeline. Anything else falls through to the caller's
        /// post-stage op check, which reports the internals a contributor needs.
        /// </summary>
        public static void ThrowIfAParamShapeNeedsAnUnhintedInput(
            InternalComputationGraph graph, IReadOnlyList<FastNode> unresolvedSites, ModelParamList inputHints)
        {
            var sites = graph.Nodes.Where(n => n.OpCode == InternalOpCodes.MODEL_PARAM_ID_REF)
                .Concat(unresolvedSites).Distinct().ToList();
            if (sites.Count == 0) return;

            var unhinted = UnhintedInputsFeedingParamInitializers(graph, sites, inputHints);
            ...
            var values = unhinted.Count == 1 ? "value" : "values";
            throw new InvalidOperationException(
                $"ToConcreteArchitecture: a parameter's definition depends on the {values} of "
                + $"{DescribeInputs(graph, unhinted)}, for which inputHints supplied no usable {values}. "
                + "Pass a sample value for every input, in order, e.g. "
                + "graph.ToConcreteArchitecture(graph.FromOrderedInputs([...])).");
        }
        ...
        /// <see cref="Process"/> performs. The backward walk follows producer edges and the
        /// close-to-open edge <see cref="FastProcessorHelper.RemoveUnreachableNodes"/> uses, and
        /// resolves LOOP_OPEN carry keys — whose FastNodeKey is no node's own — through
        ...
        private static List<int> UnhintedInputsFeedingParamInitializers(
            InternalComputationGraph graph, IReadOnlyList<FastNode> idRefNodes, ModelParamList inputHints)
        {
        ...
        /// <summary>
        /// <paramref name="paramInfos"/> ordered so that a parameter whose initializer reads
        /// another parameter's value comes after that one (Shorokoo/Shorokoo#324). Order is
        /// otherwise the input order, so a graph with no such dependency is unchanged. A cycle —
        /// two parameters each initialized from the other — has no order and is refused; every
        /// other input order is a plain topological sort.
        /// </summary>
        private static List<TrainableParamInfo> OrderParamsBySourceDependency(
            ImmutableArray<TrainableParamInfo> paramInfos)
        {
            if (paramInfos.All(x => !x.SourceParamIds.Any())) return [.. paramInfos];

            var byId = new Dictionary<ModelId, TrainableParamInfo>();
            foreach (var info in paramInfos) byId[info.SpecificModelId] = info;

            var ordered = new List<TrainableParamInfo>(paramInfos.Length);
            var placed = new HashSet<ModelId>();
            var onPath = new HashSet<ModelId>();

            void Place(TrainableParamInfo info)
            {
                if (placed.Contains(info.SpecificModelId)) return;
                if (!onPath.Add(info.SpecificModelId))
                    throw new InvalidOperationException(
                        "Parameter initialization: the parameter at ModelId "
                        + $"[{string.Join(", ", info.SpecificModelId.Vals)}] is initialized from a parameter "
                        + "that is itself initialized (directly or not) from it. A cycle has no value to "
                        + "start from, so one of the two has to be initialized on its own.");
                foreach (var sourceId in info.SourceParamIds)
                    if (byId.TryGetValue(sourceId, out var source)) Place(source);
                onPath.Remove(info.SpecificModelId);
                placed.Add(info.SpecificModelId);
                ordered.Add(info);
            }

            foreach (var info in paramInfos) Place(info);
            return ordered;
        }
            ...
            List<FastNode>? unresolvedSites = null)
        {
            ...
            foreach (var node in graph.Nodes)
            {
                ...
                foreach (var (iterIdx, filteredId) in EnumerateIterationIntVectors(modelIdRt))
                {
                    ...
                    {
                        if (initParamSources[p] is not null) { paramValues.Add(null); continue; }
                        var paramList = initParamIterations[p];
            ...
            ParamIdsByOutputKey(InternalComputationGraph graph, Dictionary<FastTensorKey, IRuntimeTensor> store)
        {
            ...
            {
                if (node.OpCode != InternalOpCodes.MODEL_PARAM_ID_REF) continue;
                if (node.Outputs.Count == 0 || node.Outputs[0] is not FastTensorKey outKey) continue;
                if (node.Inputs.Count == 0 || node.Inputs[0] is null) continue;
    ...
    /// pipeline did, bar one: full INSERT/ERASE/LENGTH/AT coverage on
    /// `knownSequences`, scan vars, dynamic cond chain via AND/WHERE, nested
    /// LOOP/IF with `GraphOpenNodeKey` rewiring, and loop-invariant body-node
    /// sharing. The exception is the per-iteration identifier-template /
    /// model-id rewrite, which has been removed rather than ported: a loop body
    /// that would need it is disqualified from the unroll instead — see
    /// <see cref="FastFoldConstantIterationLoops.BodyHoldsUnresolvedParamMachinery"/>.
    /// </para>
    /// </summary>
    internal static class FastSimplify
    {
        public static void Process(InternalComputationGraph graph, ComputeContext? context = null)
        {
            ...
            while (outerChanged)
            {
                ...
                // Native loop-unroll pass — the only unroller there is. Unrolling
                // typically turns the LOOP_OPEN's iteration-index output into a
                // scalar CONSTANT, which is why the outer loop re-enters the inner
                // native folding above after every unroll pass.
                ...
                // and loop-invariant body-node sharing. What it declines is listed
                // on that class; a declined loop stays rolled, which every
                // downstream pass accepts but two would rather not see — see its
                // Process docstring.
            }
        }
    }
    ...
    internal static class FastFoldConstantIterationLoops
    {
            ...
            long iterCount)
        {
            ...
            for (long iter = 0; iter < iterCount; iter++)
            {
                ...
                // One pass per iteration: a basic clone with remapped FullInputs and
                // fresh FullOutputs. Attributes and IdentifierTemplate are carried over
                // by reference, unchanged.
                //
                // There used to be a second pass here — Stage F, which walked each
                // clone's iteration-index chain and rewrote its ShrkAttrLocalModelId /
                // ShrkAttrRelativeModelId and IdentifierTemplate to name that iteration's
                // parameter. It is gone. Nothing that needs it reaches a clone any more:
                // inside ToConcreteArchitecture the Stage-F passes resolve every indirect
                // parameter reference before FastSimplify runs, and a body that still
                // holds one is refused the unroll outright — see
                // FastFoldConstantIterationLoops.BodyHoldsUnresolvedParamMachinery.
                //
                ...
                // Belt and braces: BodyHoldsUnresolvedParamMachinery already rejected every
                // loop that reaches here with these four in its body (a zero-trip loop is
                // exempt from that gate but clones nothing, so it never reaches here), and
                // inside ToConcreteArchitecture the Stage-F passes have resolved them
                // before FastSimplify anyway. The assert catches a body node that reached
                // a clone by some route neither covers.
