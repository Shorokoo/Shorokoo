...
namespace Shorokoo
{
    ...
    /// </list>
    ///
    /// <para><b>Dtype.</b> The optimizer's declared signature (<c>[Hyper(...)] Scalar&lt;T&gt;</c>) is the
    /// single source of truth for a hyperparameter's dtype; a hyperparameter can be any supported scalar
    /// dtype, not just <c>float32</c>. A baked value carries the dtype of whatever it was built from
    /// (<see cref="BakedDType"/>) and is converted to the declared dtype at rig build, which fails loud
    /// when the conversion would lose the value. Built-in <see cref="Schedule"/> math is continuous, so a
    /// scheduled built-in requires a <c>float32</c> declaration; a scheduler <i>module</i> may produce any
    /// declared dtype.</para>
    ///
    ...
    public readonly struct Hyperparameter
    {
        ...
        private Hyperparameter(HyperparameterKind kind, TensorData? bakedValue, Schedule? schedule, ComputationGraph? schedulerModule)
        {
            Kind = kind;
        ...
        public static Hyperparameter Baked(float value) => Baked(HyperparameterScalar.Of(value));

        /// <summary>A fixed <c>float64</c> value baked into the graph as a constant.</summary>
        public static Hyperparameter Baked(double value) => Baked(HyperparameterScalar.Of(value));

        /// <summary>A fixed <c>int32</c> value baked into the graph as a constant.</summary>
        public static Hyperparameter Baked(int value) => Baked(HyperparameterScalar.Of(value));

        /// <summary>A fixed <c>int64</c> value baked into the graph as a constant.</summary>
        public static Hyperparameter Baked(long value) => Baked(HyperparameterScalar.Of(value));

        /// <summary>A fixed <c>bool</c> value baked into the graph as a constant.</summary>
        public static Hyperparameter Baked(bool value) => Baked(HyperparameterScalar.Of(value));

        /// <summary>
        /// A fixed scalar of an explicitly chosen dtype, baked into the graph as a constant — the general
        /// form behind the per-dtype overloads, for a dtype with no natural C# literal (e.g. <c>float16</c>
        /// or <c>uint64</c>). Must be rank-0.
        /// </summary>
        public static Hyperparameter Baked(TensorData value)
        {
            ...
            if (value.Shape.Dims.Length != 0)
                throw new ArgumentException(
                    $"A baked hyperparameter must be a scalar (rank-0); got rank {value.Shape.Dims.Length}.",
                    nameof(value));
            return new(HyperparameterKind.Baked, value, null, null);
        }
        ...
        /// the shape-inference placeholder is internal, never user-visible, and the field's dtype comes
        /// from the optimizer's declaration.
        /// </summary>
        public static Hyperparameter Runtime() => new(HyperparameterKind.Runtime, null, null, null);

        /// <summary>
        /// The baked constant value, as the rank-0 <see cref="TensorData"/> it was built from. Defined
        /// only for a <see cref="HyperparameterKind.Baked"/> hyperparameter; throws otherwise (the value
        /// of a scheduled/runtime hyperparameter comes from evaluating its graph, never from this type).
        /// </summary>
    ...
    /// <summary>
    /// Scalar host values for hyperparameters: building a rank-0 <see cref="TensorData"/> from a boxed
    /// CLR primitive, reading one back, and converting one to an optimizer's declared dtype. The declared
    /// dtype is the source of truth, so every host-supplied value (a baked constant, a per-step
    /// <c>MakeHyperparameters</c> value) passes through <see cref="ConvertTo"/>, which fails loud rather
    /// than silently truncating.
    /// </summary>
    internal static class HyperparameterScalar
    {
        ...
        internal static TensorData Of(object value) => value switch
        {
            ...
            _ => throw new ArgumentException(
                ...
                "numeric or bool value, or a rank-0 TensorData of the declared dtype."),
        };

        /// <summary>The zero/false value of <paramref name="dtype"/> as a rank-0 tensor.</summary>
        internal static TensorData Zero(DType dtype) => Globals.TensorDataWithDefaultVals(dtype, []);
        ...
        internal static void AssertSupported(DType dtype, string name)
        {
            ...
            throw new ArgumentException(
                ...
                "dtype. Declare it as a numeric or bool Scalar<T>.", nameof(dtype));
        }

        /// <summary>
        /// Converts a host-supplied scalar to <paramref name="declared"/>, the optimizer's declared dtype.
        /// An exact-dtype value passes through; a convertible one is converted only when the conversion
        /// round-trips exactly, so a truncating or out-of-range value (0.5 into an int32, 300 into an
        /// int8, a bool into a float) fails loud instead of silently changing the value.
        /// </summary>
        internal static TensorData ConvertTo(TensorData value, DType declared, string name)
        {
            ...
            if (value.Shape.Dims.Length != 0)
                throw new ArgumentException(
                    $"Hyperparameter '{name}' must be a scalar (rank-0); got rank {value.Shape.Dims.Length}.",
                    nameof(value));
