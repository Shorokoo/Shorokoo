...
- `OnnxEngine.Eval(...)` is the simplest way to get values. It builds an ONNX model
  ...
  - `Eval` runs a graph of plain ops only. Nothing on its path lowers a
    module-invoke node, so a `[Module]` output handed to it throws a `ModelException`
    (`FW046`) naming the residual module ops and the lowering call. Concretize it
    first — see [Running a `[Module]`](#running-a-module).
...
- **A `[Module]`'s output, no.** `Eval` builds and runs the graph exactly as
  ...
  un-lowered module-invoke node. Shorokoo refuses that graph before OnnxRuntime
  ever sees it, with a `ModelException` (`FW046`) naming the residual ops and the
  lowering call. Lower the module's `ComputationGraph` first — see
  [Running a `[Module]`](#running-a-module), which spells the error out.
...
module-invoke node, and passing it straight to `Eval` throws a `ModelException`
with error code `FW046`, whatever the module's parameters are initialized from:

> `[FW046] Model operation failed for graph execution: this graph is still a
> module graph — it carries module machinery that no ONNX Runtime kernel
> implements (ShrkCreateModule, ShrkModelInvoke, ShrkModuleSetHyperparams).
> Passing a [Module]'s output (MyModule.Call(x)) straight to Eval is the usual
> cause: Eval runs the graph exactly as handed to it. Lower the module's
> ComputationGraph first — graph.ToConcreteArchitecture(graph.FromOrderedInputs(inputs)).ToConcreteModel()
> — and execute that.`

The ops in the parentheses are the module machinery the graph still carries;
which ones appear depends on the module. `ComputeContext.Execute`/`Compile` on a
module-stage `ComputationGraph` refuse it just as early, with the matching
`InvalidOperationException` about a graph kind
(see [The lowering pipeline](#the-lowering-pipeline)).

The remedy is what the message says. Concretize the module's `ComputationGraph`
...
takes output values rather than a `ComputationGraph`, so it has no `Kind` to check;
it scans the graph it builds for residual module machinery instead and refuses it
with `FW046` before OnnxRuntime sees the model.
