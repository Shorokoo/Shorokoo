...
public class ScheduleLoweringCoverageTests
{
    ...
    private static void AssertParity(Schedule schedule, IEnumerable<long> probeSteps, int ortUlps = 0)
    {
        ...
        var graph = InternalComputationGraphConverter.ToFastGraph(new InternalComputationGraph([steps], [value]));
    ...
    public void TestScalarLoweringMatchesAndRoundtrips()
    {
        ...
        {
            var step = InputScalar<int64>("step");
            var graph = InternalComputationGraphConverter.ToFastGraph(
                new InternalComputationGraph([step], [schedule.LowerToGraph(step)]));
        ...
        var g2 = InternalComputationGraphConverter.ToFastGraph(
            new InternalComputationGraph([s2], [schedule.LowerToGraph(s2)]));
