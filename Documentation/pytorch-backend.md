...
- **Operator coverage is partial.** The elementwise math and activation operators, the
  ...
  translated. Recurrent networks, random draws, sequences, strings, signal, quantization
  operators and `ImageDecoder` are not yet: a model using one is refused when its session is
  created, naming it.
