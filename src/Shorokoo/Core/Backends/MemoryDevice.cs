...
/// package, so every member added to <see cref="IShorokooBackend"/> is a break for
/// out-of-tree backends. The device is therefore not something a backend reports: it is derived
/// from the <see cref="IShorokooBackend.MemorySpace"/> the interface already carries, and
...
public sealed class MemoryDevice
{
    ...
    private readonly WeakSet<IShorokooBackend> _backends = new();
    ...
    public static MemoryDevice Of(IShorokooBackend backend)
    {
    ...
    /// </summary>
    public IReadOnlyList<IShorokooBackend> Backends => _backends.Snapshot();
...
/// <see cref="IShorokooBackend"/> breaks every out-of-tree implementation. The register
...
public static class BackendRegistry
{
    ...
    private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<
        IShorokooBackend, WeakSet<ComputeContext>> Contexts = new();
    ...
    /// </summary>
    /// <exception cref="ArgumentNullException"><paramref name="backend"/> is null.</exception>
    public static IReadOnlyList<ComputeContext> ContextsOn(this IShorokooBackend backend)
    {
        ArgumentNullException.ThrowIfNull(backend);
    ...
    internal static void Attach(IShorokooBackend backend, ComputeContext context)
