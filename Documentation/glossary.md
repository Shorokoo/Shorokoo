...
| `ToConcreteArchitecture` | Lowers a module graph into a concrete architecture (inlines sub-modules so trainable params are top-level) and fixes its parameter space — shapes, count, and which params exist — from the values supplied. Those inputs stay live; re-supply the same values at `Execute`. Required before `ToConcreteModel`/`InitializeTrainableParams`. | `onnx-and-weights.md`, `inference.md` |
