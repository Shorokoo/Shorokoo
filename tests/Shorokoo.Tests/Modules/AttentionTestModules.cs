...
/// graph that took ONNX Runtime down (rank-1 has no axis -2 to read or slice).
/// </summary>
[Module]
public partial class AttnChunkedUnknownRankMask
{
    public static Scalar<bit> Inline(Tensor<float32> qkv, Tensor<float32> mask)   // mask [Lk]
    {
        int[] counts = [2, 3];
...
/// <c>Documentation/defining-models.md → "Workflow: one module, many variants"</c>: one
/// [Module] whose width, depth, head count, class count, bias use and head shape are all
/// [Hyper]s, so every variant of it is a value rather than another class.
/// </summary>
[Module]
public partial class DocVariantVisionTransformer
{
        ...
        [Hyper] Scalar<bit> useMlpHead)
    {
        ...
        var pooled = (Tensor<float32>)OnnxOp.ReduceMean(
            x, Globals.Vector(1L), keepdims: false, noopWithEmptyAxes: null);
